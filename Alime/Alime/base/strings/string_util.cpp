	...
	namespace base {
		...
		namespace
		{
			...
			std::basic_string<CharType> LowerStringT(std::basic_string_view<CharType> input)
			{
				if (input.empty())
					return {};
				std::basic_string<CharType> str(input.data());
			...
			{
				std::basic_string<CharType> output = input.data();
				size_t check = 0;
				...
				RemoveT(std::basic_string_view<CharType> src, size_t startIndex, size_t count)
			{
				...
				return result + left.data() + right.data();
			}
		} // anonymous namespace
		...
		{
			return std::string(str0.data()) + str1.data();
		}

		std::wstring Concat(std::wstring_view str0, std::wstring_view str1)
		{
			return std::wstring(str0.data()) + str1.data();
		}
		...
		std::vector<std::string> Split(std::string_view input, std::string_view separator)
		{
			...
			char* token = strtok(&input2[0], separator.data());
			while (token != NULL)
			{
		...
		std::vector<std::wstring> Split(std::wstring_view input, std::wstring_view separator)
		{
...
#if defined(OS_WIN)	
			wchar_t* token = wcstok(&input2[0], separator.data());
			while (token != NULL)
			{
				output.push_back(token);
		...
		std::vector<std::string> Split(std::string_view src, std::string_view separator, std::string_view quote)
		{
			...
			StringTokenizer tokenizer(input, separator.data());
			tokenizer.set_quote_chars(quote.data());
		...
		std::vector<std::wstring> Split(std::wstring_view src, std::wstring_view separator, std::wstring_view quote)
		{
			...
			WstringTokenizer tokenizer(input, separator.data());
			tokenizer.set_quote_chars(quote.data());
