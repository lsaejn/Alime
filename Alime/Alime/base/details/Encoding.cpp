...
namespace Alime::base::System::IO
{
	...
	//fix me, Alime::SysWideCharToANSI
	...
	aint Utf8Encoder::WriteString(wchar_t* buffer, aint chars)
	{
		...
		while (src_begin < src_end)
		{
			...
			totalByteWrited += byteWrited;
			dst_begin = output;
			if (result == sourceExhausted)
			{
				assert(totalByteWrited / sizeof(wchar_t) == chars);
				return chars;
			}
			else if (result == sourceIllegal)
			{
				Close();
				return totalByteWrited / sizeof(wchar_t);
			}
	...
	aint Utf8Decoder::ReadString(wchar_t* _buffer, aint chars)
	{
		...
		while (readed < chars)
		{
			...
			else
			{
				...
				const UTF8* sourceEnd = reinterpret_cast<const UTF8*> (source + sourceCount);

				UTF16* targetU16 = reinterpret_cast<UTF16*>(target);
				UTF16* targetEndU16 = reinterpret_cast<UTF16*>(target + 2);

				ConversionResult ret = ConvertUTF8toUTF16(
					&sourceStart, sourceEnd,
					&targetU16, targetEndU16, lenientConversion);

				int targetCount = reinterpret_cast<wchar_t*>(targetU16) - target;
				wchar_t test = target[0];
